...
east_position_tf                   = feedback(east_velocity_controller_tf*east_velocity_tf, 1) * 1/s

east_position_controller_tf        = 0.25;
east_position_controller1_tf       = 0.10448 * (s + 2.393);

east_position_controller_name      = 'P Controller'
east_position_controller1_name     = 'PD Controller'

transfer_function           = east_position_controller_tf*east_position_tf;
transfer_function1          = east_position_controller1_tf*east_position_tf;
...
step(feedback(east_position_tf, 1), 'b')
step(feedback(transfer_function, 1), 'r')
step(feedback(transfer_function1, 1), 'g')

xlabel('Time (seconds)', 'FontSize', 20)
ylabel('Amplitude', 'FontSize', 20)
legend({'Unity Feedback', east_position_controller_name, east_position_controller1_name}, 'FontSize', 16);
...
[mag, phase, wout]    = bode(east_position_tf);
[Gm,Pm,Wgm,Wpm]       = margin(east_position_tf);
...
legend({'Plant', east_position_controller_name, east_position_controller1_name, 'Crossover Frequencies'},'FontSize', 16);
hold off;
...
title(['Root Locus - ' east_position_controller_name], 'FontSize', 20)
...
title(['Root Locus - ' east_position_controller1_name], 'FontSize', 20)
xlabel('Imaginary Axis', 'FontSize', 20);
ylabel('Real Axis', 'FontSize', 20);
