...
#define S_FUNCTION_NAME  update_throttle_rpy_mix
...
#define _throttle_rpy_mix_in                u[0]
#define _throttle_rpy_mix_desired           u[1]

#define _throttle_rpy_mix_out               y[0]
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    if (!ssSetNumInputPorts(S, 2)) return;                                  //Set the number of inputs, with a return excepton if there is an error
    ssSetInputPortWidth(S, 0, 1);                                           //Set the input port dimensions
    ssSetInputPortWidth(S, 1, 1);                                           //Set the input port dimensions
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    if (_throttle_rpy_mix_in < _throttle_rpy_mix_desired) 
    {
        _throttle_rpy_mix_out += _throttle_rpy_mix_desired-_throttle_rpy_mix_in;
    } 
    else if (_throttle_rpy_mix_in > _throttle_rpy_mix_desired) 
    {
        _throttle_rpy_mix_out -= _throttle_rpy_mix_in-_throttle_rpy_mix_desired;
    }
    
    if(_throttle_rpy_mix_out > 1)
    {
        _throttle_rpy_mix_out = 1;
    }
    else if (_throttle_rpy_mix_out < 0.1)
    {
        _throttle_rpy_mix_out = 0.1;
    }
